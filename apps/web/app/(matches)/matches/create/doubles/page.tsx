import DoubleMatchForm from '@web/components/double-match-form'
...
async function DoublesPage({}: Props) {
  ...
  return (
    <div className='h-[calc(100vh-64px)] flex w-full items-center justify-center'>
        <DoubleMatchForm user={session?.user!} allUsers={allUsers} token={session?.access_token as string}/>
    </div>
  )
}
