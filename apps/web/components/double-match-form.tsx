...
import { doubleMatchSchema } from '@web/lib/validations/doublematch'
...
type FormData = z.infer<typeof doubleMatchSchema>
function DoubleMatchForm({allUsers, token,user}: Props) {
    const {
        register,
        handleSubmit,
        setValue,
        ...
        defaultValues: {
            player1Id_team1: user.id.toString(),
            player2Id_team1: allUsers.filter((e) => {
                return e.id !== parseInt(user.id)
            }

            )[0].id.toString(),
            player1Id_team2: allUsers.filter((e) => {
                return e.id !== parseInt(user.id)
            }
            )[1].id.toString(),
            player2Id_team2: allUsers.filter((e) => {
                return e.id !== parseInt(user.id)
            }
            )[2].id.toString(),
        ...
        resolver: zodResolver(doubleMatchSchema),
      })
      ...
      async function onSubmit(data: FormData) {
        ...
        const doubleMatch = {
            player1Id_team1: parseInt(data.player1Id_team1),
            player2Id_team1: parseInt(data.player2Id_team1),
            player1Id_team2: parseInt(data.player1Id_team2),
            player2Id_team2: parseInt(data.player2Id_team2),
        ...
        const result = await axios.post("http://localhost:4000/doublematch", doubleMatch, {
            headers: {
  ...
  return (
    <Card className={"grid gap-6 w-[450px]"}>
        <CardHeader className='pb-0'>
            <CardTitle>Add Doubles Match</CardTitle>
        </CardHeader>
        <CardContent>
      <form            
     ...
     onSubmit={handleSubmit(onSubmit)}>
        <div className="grid gap-4">
          <div className="grid gap-3">
            ...
            <Select onValueChange={(e) => setValue("player1Id_team1",e)} defaultValue={allUsers.find((e) => {
                return e.id === parseInt(user.id)
            })?.id.toString() || "1" }>
    ...
    <SelectContent>
        {allUsers.map(user => {
            ...
            {errors?.player1Id_team1 && (
              <p className="px-1 text-xs text-red-600">
                {errors.player1Id_team1.message}
              </p>
            )}
          </div>
          <div className="grid gap-3">
            ...
            <Select onValueChange={(e) => setValue("player2Id_team1",e)} defaultValue={allUsers.find((e) => {
                return e.id === parseInt(user.id)
            })?.id.toString() || "1" }>
    <SelectTrigger className="w-full">
        <SelectValue placeholder="Player 1" />
    ...
    <SelectContent>
        {allUsers.map(user => {
            ...
            {errors?.player2Id_team1 && (
              <p className="px-1 text-xs text-red-600">
                {errors.player2Id_team1.message}
              </p>
            )}
          </div>
          ...
          <div className="grid gap-3">
            ...
            <Select onValueChange={(e) => setValue("player1Id_team2",e)} defaultValue={allUsers.find((e) => {
                return e.id === parseInt(user.id)
            })?.id.toString() || "1" }>
    <SelectTrigger className="w-full">
        <SelectValue placeholder="Player 1" />
    ...
    <SelectContent>
        {allUsers.map(user => {
            ...
            {errors?.player1Id_team2 && (
              <p className="px-1 text-xs text-red-600">
                {errors.player1Id_team2.message}
              </p>
            )}
          </div>
          <div className="grid gap-3">
            ...
            <Select onValueChange={(e) => setValue("player2Id_team2",e)} defaultValue={allUsers.find((e) => {
                return e.id === parseInt(user.id)
            })?.id.toString() || "1" }>
    <SelectTrigger className="w-full">
        <SelectValue placeholder="Player 1" />
...
export default DoubleMatchForm